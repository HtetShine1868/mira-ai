...
export interface SecurityReport {
  ...
  attackType: string;               // e.g. "Phishing", "Scam", "Fake Login", "Social Engineering", "Malware", "Safe"
  indicators: string[];             // specific red flags observed
  explanation: string;              // why this content is dangerous / safe
  recommendedActions: string[];     // actionable steps for the user
  affectedElements: string[];       // which UI parts look suspicious (URL bar, form, logo, etc.)
  severityBreakdown: {
    urgency: 'Immediate' | 'High' | 'Moderate' | 'Low' | 'None';
    dataRisk: 'High' | 'Medium' | 'Low' | 'None';        // risk of data theft
    financialRisk: 'High' | 'Medium' | 'Low' | 'None';   // risk of financial loss
  };
}
...
export interface PromptTemplate {
  ...
  promptText: string;
  responseSchema: any; // Gemini SDK structured schema
}
...
// ─────────────────────────────────────────────────────────────────────────────
export const CYBERSECURITY_TEMPLATE: PromptTemplate = {
  name: 'Mira Cyber Threat Analysis',
  description:
    'Analyses a screenshot from a simulated user environment to detect phishing, scams, fake login pages, and social engineering attacks.',
